...
def main():
    def initial_options():
        ...
        st.session_state.user_menu = streamlit_option_menu.option_menu(
            ...
            options=['Recommend me a similar movie', 'Recommend by Genre' , 'Describe me a movie', 'Check all Movies'],
        ...
        elif st.session_state.user_menu == 'Recommend by Genre':
            recommend_by_genre()
    ...
    def display_movie_details():
        ...
        if search_query:
            filtered_movies = [movie for movie in new_df['title'].values if search_query.lower() in movie.lower()]
            if filtered_movies:
                selected_movie_name = st.selectbox(
            ...
            # If no search query, show all movies in dropdown
            selected_movie_name = st.selectbox(
                'Select a movie...',
                new_df['title'].values
            )
    ...
    def recommend_by_genre():
        st.title('Movie Recommender System - By Genre')
        
        # Get unique genres from the dataset
        all_genres = set()
        for genres in new_df['genres']:
        ...
        if rec_button:
            ...
            genre_movies = new_df[new_df['genres'].str.contains(selected_genre, case=False)]
            
            if len(genre_movies) > 0:
                st.subheader(f'Top Movies in {selected_genre.capitalize()} Genre')
                
                # Display top 5 movies from the genre
                rec_movies = []
                rec_posters = []
                for _, row in genre_movies.head(5).iterrows():
                    rec_movies.append(row['title'])
                    rec_posters.append(preprocess.fetch_posters(row['movie_id']))
